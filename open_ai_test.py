...
export = "수출 실적 보유"
date = "3년 이상"
#   & Q(employee_count__contains=employee) & Q(revenue__contains=sales) & Q(business_period__contains=date)
data = BizInfo.objects.filter(
    (Q(region__contains=region) | Q(region__contains="전국")) & Q(possible_industry__contains=possible_industry) & Q(employee_count__contains=employee)
)
