...
function showFilePreviewModal(fileInfo) {
    console.log('=== showFilePreviewModal 호출됨 ===');
    console.log('전체 fileInfo:', fileInfo);
    console.log('preview_url:', fileInfo.preview_url);
    console.log('download_url:', fileInfo.download_url);
    console.log('public_url:', fileInfo.public_url);
    
    const ext = (fileInfo.original_filename || '').split('.').pop().toLowerCase();
    let viewerHtml = '';
    let isPreviewable = true;

    // 미리보기용 URL 우선 사용, 없으면 다운로드 URL 사용
    const previewUrl = fileInfo.preview_url || fileInfo.download_url || fileInfo.public_url;

    console.log('선택된 previewUrl:', previewUrl);
    console.log('파일 확장자:', ext);
    console.log('content_type:', fileInfo.content_type);

    if (fileInfo.content_type && fileInfo.content_type.startsWith('image/')) {
        viewerHtml = `<img src="${previewUrl}" style="max-width:100%; max-height:80vh; display:block; margin:auto;" />`;
        console.log('이미지 파일 처리');
    } else if (ext === 'pdf' || fileInfo.content_type === 'application/pdf') {
        ...
        const url = encodeURIComponent(fileInfo.public_url || fileInfo.download_url);
        viewerHtml = `
