...
function generateDetailModalContent(attributes, rowData, rowId) {
  ...
  sortedAttributes.forEach(function (attr) {
    ...
    if (attr.name === "지역") {
      ...
      if (!regionProcessed) {
        html += `
                    <div class="attribute-row" data-attribute-id="${
                      attr.id
                    }" data-attribute-name="지역" style="display:flex;align-items:center;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid #eee;position:relative;">
                        ...
                        <div style="flex:1;display:flex;align-items:center;gap:10px;">
                            <button type="button" class="add-btn" style="width: 50%; background:#f8f9fa;color:#333;border:1px solid #eee;padding:8px 12px;border-radius:4px;" onclick="openDetailDropdown('${rowId}','지역',this)">${
          regionValue || "선택"
        }</button>
                            <button type="button" class="add-btn" style="width: 50%; background:#f8f9fa;color:#333;border:1px solid #eee;padding:8px 12px;border-radius:4px;" onclick="openDetailDropdown('${rowId}','상세지역',this)">${
          subregionValue || "선택"
        }</button>
                        </div>
                    </div>
    ...
    if (readonlyFields.includes(attr.name)) {
      ...
      inputHtml = `
                <div style="border: 1px solid #b1b1b1; border-radius: 4px; padding: 12px; background: #fff;" data-field="${
                  attr.name
                }" data-current-value='${JSON.stringify(businessData)}'>
      ...
      const selectOptions = industryOptions
          ...
          if (fileInfo.type === "img" || fileInfo.content_type?.startsWith("image/")) {
            filesHtml += `
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; padding: 8px; border: 1px solid #000; border-radius: 4px; background: #f8f9fa;">
                                ...
                                <button onclick="showFilePreviewModal(${JSON.stringify({
                                  ...fileInfo,
                                  id: fileId,
            ...
            filesHtml += `
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; padding: 8px; border: 1px solid #e9ecef; border-radius: 4px; background: #f8f9fa;">
                                <span style="flex: 1; font-size: 14px;">📄 ${displayFileName}</span>
                                <button onclick="showFilePreviewModal(${JSON.stringify({ ...fileInfo, field_name: attr.name, row_id: rowId }).replace(
                                  /\"/g,
                                  "&quot;"
                                )})"
                                        style="padding: 4px 8px; background: #ffc107; color: #333; border: none; border-radius: 3px; cursor: pointer; font-size: 11px; font-weight: 500;">
      ...
      inputHtml = `<button type="button" class="add-btn" id="modal-dropdown-btn-${rowId}-${
        attr.name
      }" style="width:100%;background:#f8f9fa;color:#333;border:1px solid #eee;" onclick="openDetailDropdown('${rowId}','${attr.name}',this)">${
        value || "선택"
      }</button>`;
    ...
    } else if (attr.type === "datetime" && attr.name !== "경력") {
      // 날짜 형식 변환
      let dateValue = "";
      if (value) {
        try {
      ...
      inputHtml = `
                <div style="border: 1px solid #b1b1b1; border-radius: 4px; padding: 12px; background: #fff;" data-field="${
                  attr.name
                }" data-current-value='${JSON.stringify(ageData)}'>
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        <!-- 생년월일 입력 -->
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            ${(() => {
                              if (supportValue) {
                                try {
                                  ...
                                  if (typeof supportValue === "string") {
                                    // Python 딕셔너리 형태의 문자열을 JavaScript 객체로 변환
                                    const cleanedString = supportValue.replace(/'/g, '"').replace(/True/g, "true").replace(/False/g, "false");
                                    supportData = JSON.parse(cleanedString);
                                  } else if (typeof supportValue === "object" && supportValue !== null) {
                                    supportData = supportValue;
                                  } else {
      ...
      try {
        if (value && typeof value === "string") {
          experienceData = JSON.parse(value);
        } else if (value && typeof value === "object") {
          experienceData = value;
        }
      } catch (e) {
        console.error("경력 데이터 파싱 오류:", e);
        experienceData = {};
      }

      // 현재 값에 따른 표시
      if (experienceData.years) {
        displayText = `${experienceData.years}년 경력`;
      } else {
        displayText = "경력 정보 없음";
      }

      inputHtml = `
                <div style="border: 1px solid #b1b1b1; border-radius: 4px; padding: 12px; background: #fff;" data-field="${
                  attr.name
                }" data-current-value='${JSON.stringify(experienceData)}'>
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        <!-- 경력 년수 입력 -->
                        <div style="display: flex; align-items: center; gap: 10px;">
                            <label style="width: 80px; font-weight: bold; color: #495057;">경력:</label>
                            <input type="number" 
                                   id="experience_years_${rowId}" 
                                   value="${experienceData.years || ""}"
                                   placeholder="년수"
                                   min="0"
                                   max="50"
                                   style="flex: 1; padding: 8px; border: 1px solid #ced4da; border-radius: 4px;"
                                   onchange="updateExperienceField('${rowId}', '${attr.name}', 'years', this.value)">
                            
                            <!-- 년 표시 -->
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <label style="display: flex; align-items: center; gap: 3px; cursor: pointer;">
                                    <span style="font-size: 12px; color: #495057;">년 경력</span>
                                </label>
                            </div>
                        </div>
                    </div>
    ...
    if (attr.name !== "지역" && attr.name !== "상세지역") {
      ...
      html += `
                <div class="attribute-row" data-attribute-id="${attr.id}" data-attribute-name="${
        attr.name
      }" style="display:flex;align-items:center;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid #eee;position:relative;">
                    <div class="drag-handle" style="position:absolute;left:-15px;top:50%;transform:translateY(-50%);color:#ccc;cursor:move;">⋮⋮</div>
    ...
    fetch(`/sales/get_row_details/${rowId}/`)
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          const hasValidRevenue = revenueValue && revenueValue !== "" && !isNaN(parseInt(revenueValue, 10)) && parseInt(revenueValue, 10) >= 0;
            ...
            ">
                ...
                <div style="margin-bottom: 25px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px;">
                        ${Object.entries(result.detailed_funds)
                          .map(
                            ([fundName, amount]) => `
                            <div style="
...
function showFundingDetailModal(rowId, fieldName) {
  ...
  fetch(`/sales/get_row_details/${rowId}/`)
    ...
    .then((data) => {
      ...
      // 공고 정보를 가져온 후 모달 표시
      recommendedNoticesPromise
        .then((recommendedNotices) => {
          // 기존 모달이 있으면 제거
                    ...
                    ">
                        ...
                        ">
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 15px;">
                                    ${normalizedData.individual_funds
                                      .map(
                                        (fund, index) => `
                                        <div style="
                                            background: #f8f9fa;
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px;">
                                    ${Object.entries(normalizedData.detailed_funds)
                                      .map(
                                        ([fundName, amount]) => `
                                        <div style="
                                            background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%);
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 12px;">
                                    ${normalizedData.recommended_notices
                                      .map(
                                        (notice) => `
                                        <div onclick="window.open('/board/detail/${notice.pblanc_id}/', '_blank')" style="
                                            background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%);
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div style="space-y: 8px;">
                                    ${normalizedData.exclusion_notes
                                      .map(
                                        (note) => `
                                        <div style="
                                            background: #f8d7da;
  ...
  if (window.trackFileChange && fileInfo) {
    ...
    const enhancedFileInfo = {
      ...
      file_hash:
        fileInfo.file_hash ||
        fileInfo.hash ||
        `${fileInfo.filename || fileInfo.original_filename}_${fileInfo.file_size || fileInfo.size}_${fileInfo.last_modified || Date.now()}`,
              ...
              files.forEach((fileInfo, index) => {
                ...
                if (fileInfo.type === "img" || fileInfo.content_type?.startsWith("image/")) {
                  filesHtml += `
                                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; padding: 8px; border: 1px solid #e9ecef; border-radius: 4px; background: #f8f9fa;">
                                            <span style="flex: 1; font-size: 14px;">📄 ${displayFileName}</span>
                                            <button onclick="showFilePreviewModal(${JSON.stringify({ ...fileInfo, field_name: fieldName }).replace(
                                              /\"/g,
                                              "&quot;"
                                            )})"
                                                    style="padding: 4px 8px; background: #ffc107; color: #333; border: none; border-radius: 3px; cursor: pointer; font-size: 11px; font-weight: 500;">
...
// 경력 필드 업데이트 함수
function updateExperienceField(rowId, fieldName, dataType, value) {
  // 현재 저장된 경력 데이터 가져오기
  let currentExperienceData = {};
  const experienceElement = document.querySelector(`[data-field="${fieldName}"]`);

  try {
    // 기존 데이터 파싱 시도
    if (experienceElement && experienceElement.dataset.currentValue) {
      currentExperienceData = JSON.parse(experienceElement.dataset.currentValue);
    }
  } catch (e) {
    console.log("기존 경력 데이터 없음, 새로 생성");
  }

  // 년수만 처리
  if (dataType === "years") {
    currentExperienceData.years = value;
  }

  // 현재 데이터를 element에 저장
  if (experienceElement) {
    experienceElement.dataset.currentValue = JSON.stringify(currentExperienceData);
  }

  // 서버에 업데이트 요청
  const experienceDataToSend = JSON.stringify(currentExperienceData);
  detailUpdateRowField(rowId, fieldName, experienceDataToSend);
}
        ...
        if (data.success) {
          ...
          if (window.trackFileChange && data.file_info) {
            ...
            const existingFile = existingFiles.find(
              (f) => f.fileInfo?.original_filename === file.name && f.fileInfo?.file_hash === fileInfo.file_hash && f.fieldName === fieldName
            );
...
function updateDependentRows(updatedRowId, fieldName, value) {
  // 드롭다운 필드인지 확인
  const dropdownFields = (window.ATTR_FIELDS || []).filter((attr) => attr.attributeType_name === "dropdown").map((attr) => attr.name);
  ...
  window.triggerKanbanRefreshIfNeeded = async function (fieldName) {
    ...
    window._kanbanRefreshTimeout = setTimeout(() => {
      ...
      if (!window._kanbanRefreshing) {
        ...
        setTimeout(() => {
          window._kanbanRefreshing = false;
        }, 500);
      }
    }, 80);
  };
      ...
      if (fieldName === "지역" || fieldName === "상세지역") {
        ...
        if (businessElement) {
          try {
            ...
            const hasValidValue = businessData.opening_date || businessData.years_ago;
    ...
    .then((data) => {
      if (data.success) {
        ...
        if (data.data && data.data.length > 0) {
          showRecommendModal("추천 지원사업", "", false, data.data, rowId, "view");
        } else {
          showRecommendModal("추천 지원사업", "저장된 추천 지원사업이 없습니다. 먼저 추천받기를 진행해주세요.", false);
        }
      } else {
            ...
            <div style="max-height: 70vh; overflow-y: auto;">
                ...
                <div style="display: grid; gap: 15px;">
                    ${data
                      .map(
                        (biz, index) => `
                        <div class="biz-card" style="border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; background: ${
                          biz.isNew ? "#fff5f5" : "#fafafa"
                        }; ${biz.isNew ? "border-left: 4px solid #dc3545; border: 2px solid #ff6b6b;" : ""}">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; cursor: pointer;" onclick="window.open('https://namatji.com/board/detail/${
                              biz.pblanc_id
                            }/', '_blank')">
                                <h6 style="margin: 0; color: #007bff; font-weight: 600;" >${biz.title || "제목 없음"}</h6>
                                ${
                                  biz.isNew
                                    ? '<span style="color: #dc3545; font-weight: bold; font-size: 12px; background: #ff6b6b; color: white; padding: 4px 8px; border-radius: 12px;">NEW</span>'
                                    : ""
                                }
                            </div>
