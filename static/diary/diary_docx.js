...
function openAutoDocx() {
  ...
  // 혁신성장 카테고리 선택 이벤트 리스너 재설정
  setTimeout(() => {
    const innovationCategorySelect = modal.querySelector("#innovationCategory");
    if (innovationCategorySelect) {
      console.log("혁신성장 카테고리 select에 이벤트 리스너 재설정");
      // 기존 이벤트 리스너 제거 후 새로 추가
      innovationCategorySelect.removeEventListener("change", handleInnovationCategoryChange);
      innovationCategorySelect.addEventListener("change", handleInnovationCategoryChange);

      // HTML onchange 속성도 추가
      innovationCategorySelect.setAttribute("onchange", "handleInnovationCategoryChange(event)");
    }
  }, 100);
...
function updateInnovationCategories() {
  ...
  if (innovationType === "혁신형") {
    const options = ["수출 소상공인", "2년 연속 매출 10%이상 신장", "스마트 공장 도입", "강한 소상공인, 로컬 크리에이터", "소상공인 졸업후보기업"];
    ...
    const options = ["백년소공인, 백년가게", "스마트 기술", "사회적경제기업", "신사업 창업 사관학교 수료생"];
  ...
  // 이벤트 리스너 추가
  categorySelect.removeEventListener("change", handleInnovationCategoryChange);
  categorySelect.addEventListener("change", handleInnovationCategoryChange);
  categorySelect.setAttribute("onchange", "handleInnovationCategoryChange(event)");

  console.log("카테고리 업데이트 완료");
}

function handleInnovationCategoryChange(event) {
  console.log("=== handleInnovationCategoryChange 함수 호출됨 ===");
  console.log("선택된 값:", event.target.value);
  updateSmartTechDetails();
}

function updateSmartTechDetails() {
  console.log("=== updateSmartTechDetails 함수 호출됨 ===");
  ...
  const innovationCategorySelect = modal.querySelector("#innovationCategory");
  if (!innovationCategorySelect) {
    console.error("innovationCategory select를 찾을 수 없습니다.");
    return;
  }

  const innovationCategory = innovationCategorySelect.value;

  console.log("선택된 혁신성장 카테고리:", innovationCategory);

  if (innovationCategory === "스마트 기술") {
    console.log("스마트 기술 선택됨 - 세부 옵션 표시");
    showSmartTechDetails();
  } else {
    console.log("스마트 기술이 아님 - 세부 옵션 숨김");
    hideSmartTechDetails();
  }
}

function showSmartTechDetails() {
  console.log("=== showSmartTechDetails 함수 호출됨 ===");
  ...
  // 스마트 기술 세부 옵션 컨테이너 찾기 또는 생성
  let smartTechContainer = modal.querySelector("#smartTechDetails");
  console.log("기존 스마트 기술 컨테이너:", smartTechContainer);

  if (!smartTechContainer) {
    console.log("스마트 기술 컨테이너가 없음 - 새로 생성");
    const innovationCategoryField = modal.querySelector("#innovationCategory").closest(".form-group");
    console.log("혁신성장 카테고리 필드:", innovationCategoryField);

    if (innovationCategoryField) {
      smartTechContainer = document.createElement("div");
      smartTechContainer.id = "smartTechDetails";
      smartTechContainer.className = "form-group";
...
function getInnovationRecommendation() {
  ...
  // 스마트 기술을 선택했는데 세부 유형을 선택하지 않은 경우
  if (innovationCategory === "스마트 기술" && !smartTechType) {
    alert("스마트 기술 세부 유형을 선택해주세요.");
    return;
  }
...
function displayInnovationRecommendationResult(data) {
  ...
  const innovationCategoryField = modal.querySelector("#innovationCategory").closest(".form-group");
  const smartTechDetailsField = modal.querySelector("#smartTechDetails");
  ...
  if (smartTechDetailsField) smartTechDetailsField.style.display = "none";
  ...
  const resultHTML = `
    <div style="max-height: 500px; overflow-y: auto; padding-right: 10px;">
      ...
      <div style="margin-bottom: 20px;">
        ...
        ${data.smart_tech_type ? `<p><strong>스마트 기술 세부 유형:</strong> ${data.smart_tech_type}</p>` : ""}
      </div>
...
function generateInnovationDocx() {
  ...
  const smartTechType = document.getElementById("smartTechType") ? document.getElementById("smartTechType").value.trim() : "";
...
window.handleInnovationCategoryChange = handleInnovationCategoryChange;
