...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const requiredElements = ["dashboard-content", "inquiries-content", "alarms-content", "create-alarm-content", "create-alarm-form"];
...
function showPayAmounts() {
  hideAllContent();
  const payamountsContent = document.getElementById("payamounts-content");
  if (payamountsContent) {
    payamountsContent.style.display = "block";
  }
  document.querySelectorAll(".nav-link").forEach((link) => link.classList.remove("active"));
  document.querySelector('a[onclick="showPayAmounts(); return false;"]').classList.add("active");
  updateActiveNav("payamounts");
  loadPayAmounts();
}

// 전역 스코프에 함수 등록
window.showPayAmounts = showPayAmounts;
...
function loadViewCounts(page = 1) {
  ...
  const url = `/sales/diary_admin/diary_counts/?search=${encodeURIComponent(searchQuery)}&sort=${encodeURIComponent(
    sortBy
  )}&page=${page}&type=${currentCountType}`;
...
function renderUsersTable(users, currentUserId, isSuperAdmin, currentPage = 1, pageSize = 10) {
  ...
  users.forEach((user, index) => {
    ...
    if (isSuperAdmin) {
      ...
      // 일반 관리자는 읽기 전용으로 표시
      adminToggleBtn = user.is_admin ? `<span class="badge bg-danger">관리자</span>` : `<span class="badge bg-secondary">일반</span>`;
    }
    ...
    const useDateCell = `
            <div class="d-flex align-items-center">
                ...
                <button class="btn btn-sm btn-outline-primary" onclick="editUseDate(${user.id}, '${user.name || "사용자"}', '${user.email}', '${
      user.use_date || ""
    }')">
    ...
    const userNameCell = `<a href="javascript:void(0)" onclick="switchToUserAccount(${user.id}, '${user.name || "사용자"}', '${
      user.email
    }')" class="text-primary fw-bold" style="cursor: pointer; text-decoration: none;" title="클릭하여 이 계정으로 로그인 전환">
      ...
      if (alarm.files && alarm.files.length > 0) {
        ...
        alarm.files.forEach((file) => {
          ...
          fileDiv.innerHTML = `
                        ...
                        <div>
                            <button class="btn btn-sm btn-outline-primary me-1" onclick="window.open('${
                              file.download_url || file.public_url
                            }', '_blank')">
...
function handleUserTableSort(field, direction) {
  ...
  try {
    ...
    console.log(
      "Will send sort parameter:",
      currentUserSort.field && currentUserSort.direction ? `${currentUserSort.direction === "desc" ? "-" : ""}${currentUserSort.field}` : "none"
    );
    ...
    row.innerHTML = `
            ...
            <td>
                <div class="btn-group" role="group">
                    <button class="btn btn-sm btn-outline-primary" onclick="editUseDate(${user.id}, '${user.name}', '${user.email}', '${
      user.use_date || ""
    }')">
                        <i class="fas fa-calendar"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-danger" onclick="deleteUser(${user.id}, '${user.name}', '${user.email}')" ${
      user.id == 1 ? "disabled" : ""
    }>
...
// 결제금액 관리 관련 함수들
function loadPayAmounts() {
  fetch("/sales/diary_admin/payamounts/")
    .then((response) => response.json())
    .then((data) => {
      if (data.success) {
        updatePayAmountsTable(data.payamounts);
        updatePayAmountsSummary(data.payamounts.length);
      } else {
        console.error("결제금액 로드 실패:", data.message);
        showAlert("결제금액을 불러오는데 실패했습니다: " + data.message, "danger");
      }
    })
    .catch((error) => {
      console.error("결제금액 로드 오류:", error);
      showAlert("결제금액을 불러오는 중 오류가 발생했습니다.", "danger");
    });
}

function updatePayAmountsTable(payamounts) {
  const tbody = document.getElementById("payamounts-table-body");
  if (!tbody) return;

  tbody.innerHTML = "";

  if (payamounts.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">결제금액이 없습니다.</td></tr>';
    return;
  }
  ...
  payamounts.forEach((payamount) => {
    ...
    row.innerHTML = `
            <td>${payamount.id}</td>
            <td>${payamount.amount_name}</td>
            <td><strong>${payamount.formatted_price}원</strong></td>
            <td>${new Date(payamount.created_at).toLocaleDateString("ko-KR")}</td>
            <td>${new Date(payamount.updated_at).toLocaleDateString("ko-KR")}</td>
            <td>
                <button class="btn btn-sm btn-primary" onclick="editPayAmount(${payamount.id}, '${payamount.amount_name}', ${payamount.price})">
                    <i class="fas fa-edit"></i> 수정
                </button>
            </td>
        `;
...
function updatePayAmountsSummary(totalCount) {
  const summaryElement = document.getElementById("total-payamounts-display");
  if (summaryElement) {
    summaryElement.textContent = totalCount;
  }
}

function editPayAmount(id, name, currentPrice) {
  document.getElementById("edit-payamount-name").textContent = name;
  document.getElementById("edit-payamount-current-price").textContent = currentPrice.toLocaleString() + "원";
  document.getElementById("edit-payamount-price").value = currentPrice;

  // 모달에 현재 수정 중인 PayAmount ID 저장
  document.getElementById("payamountEditModal").setAttribute("data-payamount-id", id);

  new bootstrap.Modal(document.getElementById("payamountEditModal")).show();
}

// 전역 스코프에 함수들 등록
window.editPayAmount = editPayAmount;
window.loadPayAmounts = loadPayAmounts;
window.updatePayAmountsTable = updatePayAmountsTable;
window.updatePayAmountsSummary = updatePayAmountsSummary;

// 결제금액 저장 이벤트 리스너
document.addEventListener("DOMContentLoaded", function () {
  const savePayAmountBtn = document.getElementById("save-payamount-btn");
  if (savePayAmountBtn) {
    savePayAmountBtn.addEventListener("click", function () {
      const modal = document.getElementById("payamountEditModal");
      const payamountId = modal.getAttribute("data-payamount-id");
      const newPrice = document.getElementById("edit-payamount-price").value;
      ...
      fetch(`/sales/diary_admin/payamount/${payamountId}/update/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRFToken": getCookie("csrftoken"),
        ...
        .then((data) => {
          if (data.success) {
            alert("결제금액이 성공적으로 수정되었습니다.");
            bootstrap.Modal.getInstance(modal).hide();
            loadPayAmounts(); // 테이블 새로고침
          } else {
            alert("결제금액 수정에 실패했습니다: " + data.message);
          }
        })
        .catch((error) => {
          console.error("결제금액 수정 오류:", error);
          alert("결제금액 수정 중 오류가 발생했습니다.");
        });
    });
  }
});
