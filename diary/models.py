...
class ClassForm(models.Model):
    ...
    desired_date = models.CharField(max_length=100, blank=True, default='')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.name} - {self.phone} ({self.desired_date or '미지정'})"
...
class CountUserIP(models.Model):
    ip = models.CharField(max_length=50)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='count_user_ips')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
