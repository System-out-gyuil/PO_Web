...
from .models import Attribute, AttributeValue, User, DropdownAttribute
import json
...
def add_attribute(request):
    ...
    if request.method == 'POST':
        try:
            ...
            attribute_type = data.get('type', 'text')
            ...
            try:
                attr_type = AttributeType.objects.get(name=attribute_type)
            except AttributeType.DoesNotExist:
                return JsonResponse({
                    'success': False,
                    'error': f'지원하지 않는 속성 타입입니다: {attribute_type}'
                })
            
            # 중복 확인
            if Attribute.objects.filter(user=user, name=name).exists():
                return JsonResponse({
                    'success': False,
                    'error': f'이미 존재하는 속성명입니다: {name}'
                })
            ...
            attribute = Attribute.objects.create(
                ...
                sort_order=Attribute.objects.filter(user=user).count() + 1
            )
...
def update_attribute_name(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # 속성 찾기
            try:
                attribute = Attribute.objects.get(user=user, name=old_name)
            except Attribute.DoesNotExist:
                return JsonResponse({
                    'success': False,
                    'error': f'속성 "{old_name}"을 찾을 수 없습니다.'
                })
            
            # 새 이름으로 중복 확인
            if Attribute.objects.filter(user=user, name=new_name).exclude(id=attribute.id).exists():
                return JsonResponse({
                    'success': False,
                    'error': f'이미 존재하는 속성명입니다: {new_name}'
                })
